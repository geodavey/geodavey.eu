...
export default {
  mounted: function() {
    let width = this.$data.nativeWidth;
    ...
    // silhouette
    let silW = this.$data.silhouette.width * this.$data.silhouette.scale;
    let silH = this.$data.silhouette.height * this.$data.silhouette.scale;

    let silTransform = `translate(${width / 2 - silW / 2} ${width -
    ...
    this.rotation = interval(elapsed => {
      proj.rotate([
        -this.$data.speed * elapsed + this.$data.xOffset,
        this.$data.vTilt,
        this.$data.hTilt
      ]);
